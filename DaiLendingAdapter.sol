...
contract DaiLendingAdapter{
    ...
    function GetYDaiBalance(address account) public view returns (uint){
        return yDai.balanceOf(account);
    }
    
    function GetDaiBalance(address account) public view returns (uint){
    ...
    */
    function save(uint amount, address account) public{
    ...
    function GetGrossRevenue(address account) public view returns(uint){
        
        //  Get the price per full share
        uint price = yDai.getPricePerFullShare();
        
        //  Get the balance of yDai in this users address
        uint balanceShares = yDai.balanceOf(account);
        
        return balanceShares.mul(price);
    }
    
    function GetNetRevenue(address account) public view returns(uint){
        ...
        uint grossBalance =  balanceShares.mul(price) ; //   value in 10 ^ 36
        
        uint userDaiDepositBalance = userDaiDeposits[account].mul(1e18); // multiply dai deposit by 1 * 10 ^ 18 to get value in 10 ^36
    ...
    function Withdraw(uint amount, address owner) public{
        ...
        uint balanceShares = yDai.balanceOf(owner);
    ...
    function _save(uint amount, address account) internal{
        ...
        uint shares = yDai.balanceOf(address(this));
